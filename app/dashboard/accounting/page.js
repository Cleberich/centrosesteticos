...
export default function AccountingPage() {
  ...
  const accountingData = useMemo(() => {
    ...
    const servicePriceMap = Object.fromEntries(
      (esteticaData.services || []).map((service) => [
        String(service.id),
    ...
    const filteredApps = esteticaData.appointments.filter((app) => {
      const rawDate = app.paidAt || app.date || app.start;
      const isProcessed = ["done", "processed", "completed", "paid"].includes(
        app.status,
      );
      if (!rawDate || !isProcessed) return false;
      const appDate = new Date(rawDate);
      const appDateStr = appDate.toLocaleDateString("sv-SE");
      if (view === "dia") return appDateStr === hoyStr;
      if (view === "personalizado") return appDateStr === selectedCustomDate;
    ...
    filteredApps.forEach((app) => {
      const specName = app.specialist || "Sin asignar";
      const serviceTotal = (app.selectedServiceIds || []).reduce(
        (total, serviceId) => total + (servicePriceMap[String(serviceId)] || 0),
        0,
      );
      const monto = Number(app.total) || serviceTotal;
      const method = app.paymentMethod || app.payment_type || "unknown";
      const rate = commissionMap[specName] || 40;
      if (!specialistStats[specName]) {
        specialistStats[specName] = {
          name: specName,
