...
export async function confirmPayment(payment) {
  ...
  const specialistLimit = SPECIALIST_LIMITS[planType] || 1;
  ...
  await businessRef.update({
    ...
    specialists: (businessData.specialists || []).map((specialist, index) => ({
      ...specialist,
...
export async function syncSubscription(subscription) {
  ...
  if (subscription.status === "authorized") {
    ...
    const specialistLimit = SPECIALIST_LIMITS[planId] || 1;
    ...
    const businessSnapshot = await getAdminDb()
      .collection("centros_estetica")
      .doc(userId)
      .get();
    update.specialists = (businessSnapshot.data()?.specialists || []).map(
      (specialist, index) => ({ ...specialist, active: index < specialistLimit }),
    );
  }
