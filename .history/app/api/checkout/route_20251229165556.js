...
export async function POST(request) {
  try {
    ...
    const response = await fetch(
      "https://api.mercadopago.com/checkout/preferences",
      {
        ...
        body: JSON.stringify({
          ...
          // Las credenciales de prueba a veces fallan si el external_reference es muy largo
          external_reference: userId.substring(0, 15),
          back_urls: {
    ...
    if (!response.ok) {
      console.error("ERROR MP DETALLADO:", data);
      return NextResponse.json({ error: data.message }, { status: 400 });
    }

    // init_point es para producción, sandbox_init_point es para pruebas
    // Usamos el que esté disponible según el token
