...
export async function POST(request) {
  try {
    ...
    const response = await fetch(
      "https://api.mercadopago.com/checkout/preferences",
      {
        ...
        body: JSON.stringify({
          ...
          // IMPORTANTE: external_reference ayuda a identificar al usuario después
          external_reference: userId,

          // CORRECCIÓN AQUÍ: Aseguramos que las URLs sean absolutas
          back_urls: {
            success: "http://localhost:3000/dashboard/settings",
            failure: "http://localhost:3000/dashboard/settings",
            pending: "http://localhost:3000/dashboard/settings",
          },

          // Si activas auto_return, la URL de 'success' DEBE ser idéntica a la de arriba
          auto_return: "approved",
        }),
      }
    );
    ...
    // Para credenciales TEST, usamos sandbox_init_point
    return NextResponse.json({
