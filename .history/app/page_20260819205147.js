...
//             © 2025 AppEstetica. Elevando el estándar de belleza.
//           </p>
...
export default function EsteticaCRM() {
  const [activeTab, setActiveTab] = useState("schedule"); // schedule, clients, services, payments
  const [selectedCategory, setSelectedCategory] = useState("all");

  // Estado Mock para Citas
  const [appointments, setAppointments] = useState([
    {
      ...
      pillClass: "bg-emerald-200/60 text-emerald-800",
    },
    {
      id: "APT-002",
      ...
      pillClass: "bg-rose-200/60 text-rose-800",
    },
    {
      id: "APT-003",
  ...
  const [clients] = useState([
    {
      ...
      alerts: [
        "Alergia a adhesivos de cianocrilato (usar pegamento Sensitive)",
      ],
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF9F8] text-slate-800 font-sans">
      ...
      <aside className="w-64 min-h-screen bg-white/80 backdrop-blur-md border-r border-slate-100 p-6 flex flex-col justify-between shrink-0">
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-3 px-2">
            <div className="w-9 h-9 rounded-2xl bg-emerald-100 text-emerald-600 flex items-center justify-center font-black text-lg">
              ✦
            </div>
            <div>
        ...
        {/* Promo Widget en Sidebar */}
        <div className="space-y-4">
          <div className="bg-rose-50 border border-rose-100 p-4 rounded-3xl text-center space-y-2">
            <div className="w-8 h-8 rounded-full bg-rose-100 text-rose-500 mx-auto flex items-center justify-center">
              <Heart size={16} />
            </div>
            <p className="text-xs font-bold text-rose-900">Módulo WhatsApp</p>
            <p className="text-[10px] text-rose-700/80 leading-relaxed">
              Recordatorios automáticos activados para reducir inasistencias.
            </p>
          </div>

          <button className="flex items-center gap-3 px-4 py-3 text-xs font-bold text-slate-400 hover:text-rose-500 transition-colors w-full">
            <LogOut size={18} />
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">
              {activeTab === "schedule" && "Agenda de Turnos"}
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 bg-emerald-400 hover:bg-emerald-500 text-slate-950 font-bold text-xs px-5 py-3 rounded-2xl transition-all shadow-sm">
              <Plus size={16} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {appointments
                ...
                .map((app) => (
                  <div
                    key={app.id}
                    className={`p-6 rounded-[2rem] border ${app.bgClass} flex flex-col justify-between space-y-4 hover:shadow-md transition-all`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-extrabold bg-white/80 backdrop-blur-sm">
                        <Clock size={12} /> {app.time} ({app.duration})
                      </span>
                      <span
                        className={`text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full ${app.pillClass}`}
                      >
                        {app.category}
                      </span>
                    </div>
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-600 font-extrabold text-base flex items-center justify-center">
                      {cli.name
                        .split(" ")
                        .map((n) => n[0])
                        .join("")}
