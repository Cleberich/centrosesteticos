...
export default function SuperAdminPage() {
  ...
  const fetchBarberias = async () => {
    try {
      ...
      // Obtenemos los datos extendidos (Barberos y Reservas) para cada barbería
      const barberiasExtended = await Promise.all(
        querySnapshot.docs.map(async (docSnap) => {
          const data = docSnap.data();

          // Contar Barberos
          const barberosSnap = await getDocs(
            collection(db, "barberias", docSnap.id, "barbers")
          );

          // Contar Reservas Totales
          const reservasSnap = await getDocs(
            collection(db, "barberias", docSnap.id, "appointments")
          );

          return {
            id: docSnap.id,
            ...data,
            barberosCount: barberosSnap.size,
            reservasCount: reservasSnap.size,
          };
        })
      );

      setBarberias(barberiasExtended);
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#0a0f1a] text-slate-100 p-6 md:p-12 relative">
      ...
      <main className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 gap-4">
          {/* Header con columnas ajustadas para incluir estadísticas */}
            ...
            <form onSubmit={handleManualUpdate} className="space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-[9px] text-blue-500/70 font-bold uppercase ml-1">
                  * El vencimiento se fijará +30 días.
                </p>
              </div>
              <button
                type="submit"
