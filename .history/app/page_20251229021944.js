...
const HomePage = () => {
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-50">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            <div className="flex flex-col gap-6 lg:w-1/2 text-center lg:text-left">
              ...
              <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight">
                Gestiona tu barbería como un <span className="text-blue-600">profesional</span>
              </h1>
              <p className="text-lg sm:text-xl text-slate-600 dark:text-slate-400 max-w-xl mx-auto lg:mx-0">
                Olvídate del papel y lápiz. Consigue más clientes, automatiza tus reservas y controla tus finanzas con nuestra plataforma todo en uno.
              </p>
              ...
              <p className="text-sm text-slate-500 italic">* No requiere tarjeta de crédito</p>
            </div>
              ...
              <div className="relative aspect-[4/3] bg-slate-200 dark:bg-slate-800 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="absolute bottom-6 left-6 right-6 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm p-4 rounded-xl shadow-xl border border-white/20 transform transition-transform group-hover:-translate-y-2">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">Nueva Reserva Confirmada</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400">Carlos M. • Corte y Barba • Hoy 16:00</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Benefits Section */}
      <section id="funcionalidades" className="py-24 bg-slate-50 dark:bg-slate-900/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto mb-16 text-lg">
            Herramientas diseñadas específicamente para el flujo de trabajo de una barbería moderna.
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: <Calendar className="w-8 h-8" />, title: "Agenda 24/7", desc: "Tus clientes reservan a cualquier hora desde su móvil, sin llamadas." },
              { icon: <ShieldCheck className="w-8 h-8" />, title: "Pagos Seguros", desc: "Reduce las faltas solicitando pagos anticipados o señas online." },
              { icon: <Megaphone className="w-8 h-8" />, title: "Marketing Automatizado", desc: "Envía recordatorios por WhatsApp y recupera clientes inactivos." },
            ].map((feature, i) => (
              <div key={i} className="bg-white dark:bg-slate-800 p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1">
      ...
      <section id="precios" className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-16">Planes que escalan contigo</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <PlanCard 
              ...
              price="49" 
              popular={true} 
              features={["Agenda Ilimitada", "5 Barberos", "WhatsApp Marketing", "Reportes PDF"]} 
            />
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800 py-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-sm text-slate-500">© 2025 BarberManager. Hecho para profesionales del corte.</p>
        </div>
      </footer>
    </div>
  );
...
const PlanCard = ({ name, price, features, popular = false }) => (
  <div className={`p-8 rounded-2xl border ${popular ? 'border-blue-600 ring-4 ring-blue-600/10' : 'border-slate-200 dark:border-slate-800'} bg-white dark:bg-slate-900 relative flex flex-col`}>
    {popular && <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-4 py-1 rounded-full text-xs font-bold uppercase">Más Elegido</span>}
    ...
    <ul className="space-y-4 mb-8 flex-1">
      {features.map((f, i) => (
        <li key={i} className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300 text-left">
          <Check size={18} className="text-green-500 shrink-0" />
          {f}
        </li>
      ))}
    </ul>
    <button className={`w-full py-3 rounded-xl font-bold transition-all ${popular ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200'}`}>
      Seleccionar Plan
    </button>
  </div>
);

export default HomePage;import Link from "next/link";
import { 
  Scissors, 
  Menu, 
  CheckCircle2, 
