...
export default function DashboardPage() {
  ...
  const handleAddAppointment = async (e) => {
    ...
    const newApp = {
      ...
      // Cálculo del día actual (0-6 empezando lunes para tu agenda)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm">
              {team.length > 0 ? (
                team.map((barber, idx) => (
                  <StaffItem key={idx} name={barber.name} status="En turno" />
                ))
              ) : (
...
function StaffItem({ name, status }) {
  return (
    <div className="flex items-center gap-3 p-2">
      <div className="size-10 rounded-2xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center font-black text-blue-600 uppercase italic">
        {name.charAt(0)}
      </div>
      <div className="flex-1">
