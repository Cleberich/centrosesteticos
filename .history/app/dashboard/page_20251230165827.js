...
export default function DashboardPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [appointmentToComplete, setAppointmentToComplete] = useState(null); // Para el modal de pago
  const [searchQuery, setSearchQuery] = useState("");
  ...
  // --- NUEVA LÓGICA: FINALIZAR CON PAGO ---
  const confirmPaymentAndComplete = async (paymentMethod) => {
    ...
    try {
      ...
      await updateDoc(barberiaRef, { appointments: newList });
      setBarberiaData((prev) => ({ ...prev, appointments: newList }));
      setAppointmentToComplete(null); // Cerrar modal
    } catch (error) {
            ...
            <div className="grid gap-3">
              {filteredAppointments.map((app) => (
                <AppointmentItem
                  ...
                  app={app}
                  onDelete={() => handleDeleteAppointment(app.id)}
                  onComplete={() => setAppointmentToComplete(app)} // Abre modal de pago
                />
              ))}
            </div>
          </div>
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        ...
                        window.open(
                          `https://wa.me/${formatted}?text=Hola!`,
                          "_blank"
                        );
                        setSentMessages([...sentMessages, client.id]);
                      }}
                      className={`p-2 rounded-xl ${
                        isSent ? "bg-slate-200" : "bg-emerald-500 text-white"
                      }`}
                    >
                      {isSent ? (
                        <Check size={14} />
      ...
      {appointmentToComplete && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 border border-slate-200 dark:border-slate-800">
            ...
            <div className="grid gap-3">
              <button
                onClick={() => confirmPaymentAndComplete("Efectivo")}
              ...
              >
                <div className="bg-white dark:bg-slate-700 p-2 rounded-xl group-hover:bg-emerald-500">
                  <Banknote
                    size={20}
              ...
              <button
                onClick={() => confirmPaymentAndComplete("Transferencia")}
              ...
              >
                <div className="bg-white dark:bg-slate-700 p-2 rounded-xl group-hover:bg-blue-500">
                  <SmartphoneNfc
                    size={20}
              ...
              <button
                onClick={() => confirmPaymentAndComplete("Mercado Pago")}
              ...
              >
                <div className="bg-white dark:bg-slate-700 p-2 rounded-xl group-hover:bg-violet-500">
                  <CreditCard
                    size={20}
              ...
              <button
                onClick={() => confirmPaymentAndComplete("POS / Tarjeta")}
                className="flex items-center gap-4 w-full p-4 bg-slate-50 dark:bg-slate-800 hover:bg-violet-600 hover:text-white rounded-2xl transition-all group"
              >
                <div className="bg-white dark:bg-slate-700 p-2 rounded-xl group-hover:bg-violet-500">
                  <CreditCard
            ...
            <button
              ...
              className="w-full mt-6 text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em] hover:text-red-500 transition-colors"
            >
              Cancelar
            </button>
          </div>
