...
export default function DashboardPage() {
  ...
  const { filteredAppointments, activeTodayCount } = useMemo(() => {
    ...
    // 1. Obtenemos hoy en formato "YYYY-MM-DD" (Ej: 2025-12-30)
    ...
    const todayPending = allApps.filter((app) => {
      ...
      if (app.date) {
        if (typeof app.date === "string") {
          // Si es "2025-12-30T10:00:00" o "2025-12-30", tomamos los primeros 10 caracteres
          appDateStr = app.date.substring(0, 10);
        } else if (app.date.seconds) {
          // Si es un Timestamp de Firebase (común en el Calendar)
          appDateStr = new Date(app.date.seconds * 1000).toLocaleDateString(
            "sv-SE"
          );
        } else if (app.date instanceof Date) {
          // Si es un objeto Date de JS
          appDateStr = app.date.toLocaleDateString("sv-SE");
        }
      }

      // 3. COMPARACIÓN FINAL
      // Comparamos el string limpio contra el string de hoy
      return appDateStr === hoyStr;
    });
