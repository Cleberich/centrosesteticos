...
export default function AccountingPage() {
  ...
  // --- VISTA DE BLOQUEO SI EL PIN ESTÁ ACTIVADO ---
  if (!isAuthorized) {
    return (
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] dark:bg-[#0a0f1a] p-6 lg:p-10 overflow-y-auto transition-colors duration-300">
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-10">
        ...
        <div className="flex flex-wrap items-center gap-4">
          {/* Botón opcional para volver a bloquear si el PIN está activo */}
          {barberiaData?.useAccountingPin && (
            <button
              onClick={() => setIsAuthorized(false)}
              className="px-4 py-2 bg-slate-200 dark:bg-slate-800 rounded-xl text-[10px] font-bold uppercase dark:text-white"
            >
              Bloquear
            </button>
          )}
