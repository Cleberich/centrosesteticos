...
export default function AccountingPage() {
  ...
  // --- LÓGICA VERIFICACIÓN PIN ---
  const handleVerifyPin = (e) => {
    e.preventDefault();
    const correctPin = barberiaData?.adminPin || "1234"; // PIN por defecto si no existe en DB
  ...
  // --- VISTA DE BLOQUEO (Lo que ven los barberos) ---
  if (!isAuthorized) {
    return (
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] dark:bg-[#0a0f1a] p-6 lg:p-10 overflow-y-auto transition-colors duration-300">
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-10">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            onClick={() => setIsAuthorized(false)} // Botón para volver a bloquear
            className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all"
          >
            Bloquear
          </button>
          <button
      ...
      {/* El resto de tu tabla y gráficos se mantienen igual... */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 flex flex-col items-center justify-center text-center h-fit">
          <div className="size-16 bg-blue-50 dark:bg-blue-900/20 rounded-full flex items-center justify-center text-blue-600 mb-4">
            <TrendingUp size={28} strokeWidth={3} />
          </div>
          <h3 className="text-lg font-black dark:text-white uppercase italic">
          ...
          <p className="text-xs text-slate-500 mb-6">
            Retención del local sobre el bruto.
          </p>
          <div className="w-full space-y-2">
            <div className="flex justify-between text-[10px] font-bold uppercase text-slate-400">
              <span>Local</span>
...
// Subcomponente KpiCard se mantiene igual
function KpiCard({ label, value, icon, color }) {
  const colors = {
    blue: "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400",
    purple:
