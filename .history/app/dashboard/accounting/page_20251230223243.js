...
export default function AccountingPage() {
      ...
      if (user) {
        try {
          ...
          if (docSnap.exists()) {
            ...
            // LÓGICA DE AUTO-AUTORIZACIÓN:
            // Si la función de PIN está desactivada, autorizamos automáticamente
            if (!data.useAccountingPin) {
  ...
  const handleVerifyPin = (e) => {
    e.preventDefault();
    if (pinInput === barberiaData?.adminPin) {
      setIsAuthorized(true);
      setPinError(false);
    } else {
      setPinError(true);
  ...
  if (!isAuthorized) {
    return (
      <div className="h-full flex items-center justify-center bg-white dark:bg-[#0a0f1a] p-6">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-10 shadow-xl text-center transition-all">
          ...
          <h1 className="text-2xl font-black uppercase italic text-slate-900 dark:text-white tracking-tighter mb-2">
            Contabilidad <span className="text-blue-600">Protegida</span>
          </h1>
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-8 leading-tight">
            Ingresa el PIN de administrador para ver las finanzas
          </p>
          ...
          <form onSubmit={handleVerifyPin} className="space-y-4">
            <div className="relative">
              <input
                ...
                } rounded-2xl py-4 px-6 text-center text-xl font-black tracking-[0.5em] text-slate-900 dark:text-white outline-none focus:border-blue-600 transition-all`}
                maxLength={4}
              />
              <button
                type="button"
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black uppercase py-4 rounded-2xl transition-all shadow-lg tracking-widest text-xs active:scale-95"
            >
              Desbloquear
            </button>
          </form>
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] dark:bg-[#0a0f1a] p-6 lg:p-10 overflow-y-auto transition-colors duration-300">
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-10">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight italic uppercase">
            Contabilidad
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
          {/* BOTÓN VOLVER A BLOQUEAR (Solo si el PIN está activo) */}
          {barberiaData?.useAccountingPin && (
            <button
              onClick={() => setIsAuthorized(false)}
              className="px-4 py-2.5 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all shadow-sm"
            >
              Cerrar Sesión
            </button>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 flex flex-col items-center justify-center text-center h-fit shadow-sm">
          ...
          <h3 className="text-lg font-black dark:text-white uppercase italic tracking-tight">
            Rentabilidad
          </h3>
          <p className="text-[10px] font-bold text-slate-500 uppercase mb-6 leading-tight">
            Tu porcentaje de ganancia real sobre el bruto.
          </p>
          <div className="w-full space-y-3">
            ...
            <div className="w-full h-3 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden border dark:border-slate-700">
              <div
...
function KpiCard({ label, value, icon, color }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:scale-[1.02] active:scale-95">
      ...
      <h2 className="text-3xl font-black text-slate-900 dark:text-white mt-1 tracking-tighter italic">
        {value}
      </h2>
    </div>
  );
