...
export default function DashboardPage() {
  ...
  const handleSendReminder = (app) => {
    ...
    const msg = encodeURIComponent(
      `Hola ${app.customer}, te recordamos tu turno de hoy a las ${app.start}. ¡Te esperamos!`
    );
    window.open(`https://wa.me/${formattedPhone}?text=${msg}`, "_blank");
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex-1 overflow-y-auto p-2 md:p-8 space-y-8">
        <div className="flex overflow-scroll  md:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard
            title="Hoy"
...
function StatCard({ title, value, change, icon, color }) {
  const colors = {
    emerald:
      "text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 w-40 md:w-auto",
    blue: "text-blue-500 bg-blue-50 dark:bg-blue-500/10 w-40 md:w-auto",
    violet: "text-violet-500 bg-violet-50 dark:bg-violet-500/10 w-40 md:w-auto",
    amber: "text-amber-500 bg-amber-50 dark:bg-amber-500/10 w-40 md:w-auto",
  };
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
      <div className="flex justify-between items-start mb-4">
