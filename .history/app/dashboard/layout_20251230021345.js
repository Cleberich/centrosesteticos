...
export default function DashboardLayout({ children }) {
  ...
  // --- 2. FUNCIÓN PARA CAMBIAR TEMA Y ACTUALIZAR BASE DE DATOS ---
  const toggleDarkMode = async () => {
    ...
    // Guardar en localStorage como respaldo rápido
      ...
      <aside className="hidden lg:flex flex-col w-72 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-full p-6">
        <div className="flex flex-col h-full justify-between">
          ...
          <div className="space-y-1 pt-6 border-t border-slate-100 dark:border-slate-800">
            {/* BOTÓN TOGGLE CON ACTUALIZACIÓN DE BASE DE DATOS */}
            <button
              onClick={toggleDarkMode}
              className="w-full flex items-center justify-between px-3 py-3 rounded-xl transition-all font-medium text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 mb-2"
            >
