...
export default function DashboardPage() {
  ...
  // --- LÓGICA DE FILTRADO INTELIGENTE (Dashboard + Calendar) ---
  const { filteredAppointments, activeTodayCount } = useMemo(() => {
    const allApps = barberiaData?.appointments || [];

    // 1. Obtenemos la fecha de hoy y normalizamos a medianoche para comparar
    const hoy = new Date();
    ...
    const todayPending = allApps.filter((app) => {
      ...
      // 2. Normalización de la fecha de la cita (Venga de donde venga)
      let appDate;

      if (app.date) {
        // Si viene del Dashboard (YYYY-MM-DD) o Calendar con guiones/barras
        // Reemplazamos guiones por barras para evitar errores de zona horaria en JS
        const dateString = String(app.date).replace(/-/g, "/");
        appDate = new Date(dateString);
      } else if (app.start && String(app.start).includes("-")) {
        // Si el Calendar guarda la fecha dentro de 'start'
        appDate = new Date(String(app.start).replace(/-/g, "/"));
      } else {
        // Si no tiene fecha válida, no la mostramos en "Hoy"
        return false;
      }

      // Normalizamos la fecha de la cita a medianoche
      appDate.setHours(0, 0, 0, 0);

      // 3. Comparamos si es hoy
      return appDate.getTime() === hoy.getTime();
    });

    // 4. Búsqueda por nombre y orden por hora
