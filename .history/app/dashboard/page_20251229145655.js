...
import {
  ...
  Phone,
  MessageSquare,
  User, // Importado para el fallback
} from "lucide-react";
// Firebase
              ...
              {team.length > 0 ? (
                team.map((barber, idx) => (
                  <StaffItem
                    key={idx}
                    name={barber.name}
                    imageUrl={barber.imageUrl} // Pasamos la imagen
            ...
            <form onSubmit={handleAddAppointment} className="p-8 space-y-4">
              <div className="space-y-4">
                <input
                  required
                  name="clientName"
                  className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 font-bold dark:text-white placeholder:text-slate-400"
  ...
  return (
    <div className="flex items-center gap-4 p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.8rem] hover:border-blue-500 transition-all group shadow-sm">
      ...
      <div className="flex-1">
        <h5 className="font-black text-sm uppercase  dark:text-white leading-none">
          {name}
        </h5>
      ...
      {phone && (
        <div className="flex items-center gap-2 sm:opacity-0 group-hover:opacity-100 transition-opacity">
          <a
            href={`tel:${cleanPhone}`}
            className="p-2.5 bg-slate-50 dark:bg-slate-800 text-slate-500 rounded-xl hover:bg-blue-600 hover:text-white transition-all"
          >
          ...
          <a
            ...
            className="p-2.5 bg-slate-50 dark:bg-slate-800 text-emerald-500 rounded-xl hover:bg-emerald-500 hover:text-white transition-all"
          >
...
// --- STAFF ITEM MODIFICADO ---
function StaffItem({ name, status, imageUrl }) {
  return (
    <div className="flex items-center gap-3 p-2">
      <div className="size-10 rounded-2xl overflow-hidden bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center border border-slate-100 dark:border-slate-800">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={name}
      ...
      {status === "En turno" && (
        <div className="size-2 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/40 animate-pulse"></div>
      )}
    </div>
  );
}
