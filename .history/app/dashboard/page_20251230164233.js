...
import {
  ...
  MessageSquare,
  Trash2,
  CheckCircle2, // Icono para completar cita
  Zap, // Icono para Smart Recall
  Award, // Icono para Fidelización
  TrendingUp,
} from "lucide-react";
// Firebase
import { auth, db } from "@/services/firebase";
...
export default function DashboardPage() {
  ...
  // --- 3. LÓGICA DE COMPLETAR CITA (MARKETING) ---
  ...
  // --- 5. FILTRADO Y MÉTRICAS DE MARKETING ---
  const { filteredAppointments, activeTodayCount, inactiveClients } =
    useMemo(() => {
      const allApps = barberiaData?.appointments || [];
      ...
      // Smart Recall: Clientes que no vienen hace más de 20 días
      const lastVisits = {};
      allApps.forEach((app) => {
        if (app.status === "completed" && app.phone) {
      ...
      const inactive = Object.values(lastVisits).filter(
        (client) => client.date < twentyDaysAgo
      );
    ...
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            {/* SECCIÓN DE MARKETING: SMART RECALL */}
            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 flex items-center gap-2">
              <Zap size={14} className="text-amber-500" /> Recuperar Clientes
            </h3>
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm">
              {inactiveClients.length > 0 ? (
                inactiveClients.slice(0, 5).map((client, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2 border-b border-slate-50 dark:border-slate-800 last:border-0"
                  >
                    <div>
                      ...
                      <p className="text-[8px] font-bold text-slate-500 uppercase">
                        Último corte: {client.date.toLocaleDateString()}
                      </p>
                    </div>
                    <a
                      href={`https://wa.me/${client.phone.replace(
                        ...
                        `¡Hola ${client.name}! Hace tiempo no te vemos. ¿Te gustaría agendar un turno para esta semana?`
                      )}`}
                      target="_blank"
...
function AppointmentItem({ app, onDelete, onComplete }) {
  ...
  return (
    <div className="flex items-center gap-4 p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.8rem] hover:border-blue-500 transition-all shadow-sm">
      ...
      <div className="flex items-center gap-2">
        {/* BOTÓN COMPLETAR - ACTIVA FIDELIZACIÓN */}
        <button
          onClick={onComplete}
          className="p-3 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
        >
