...
export default function DashboardPage() {
  ...
  const handleAddAppointment = async (e) => {
    ...
    const newApp = {
      ...
      phone: formData.get("clientPhone"), // Agregado
      barber: formData.get("barberName"),
      selectedServiceIds: [],
      service: formData.get("service") || "Corte General",
                ...
                {filteredAppointments.length > 0 ? (
                  filteredAppointments.map((app) => (
                    <AppointmentItem
                      ...
                      phone={app.phone} // Pasamos el teléfono
...
// Actualizado para mostrar el teléfono
    ...
    <div className="flex items-center gap-4 p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] hover:border-blue-500 transition-all">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          {phone && (
            <span className="text-[9px] font-bold text-blue-500/60 flex items-center gap-1">
              <Phone size={8} /> {phone}
            </span>
          )}
        </div>
