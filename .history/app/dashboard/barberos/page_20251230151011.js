...
export default function BarbersPage() {
  ...
  // --- FUNCIÓN DE COMPRESIÓN DE IMAGEN CORREGIDA ---
  const handleImageUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Redimensionar a un tamaño pequeño pero nítido
        const MAX_WIDTH = 300;
        ...
        // Comprimir a JPEG con calidad 0.6 (muy ligero)
        const compressedBase64 = canvas.toDataURL("image/jpeg", 0.6);

        // Verificar si la cadena Base64 resultante es aceptable
        if (compressedBase64.length > 500000) {
          alert(
            "La imagen procesada sigue siendo muy grande. Intenta con otra."
          );
          return;
        }
  ...
  const saveToFirebase = async (newBarbersList) => {
    ...
    try {
      ...
      if (
        ...
        alert(
          "Error: La imagen es demasiado pesada para la base de datos. Intenta con una foto más pequeña o sin foto."
        );
      } else {
      ...
      {barbers.length >= PLAN_LIMITS[plan] && (
        <div className="mx-8 mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            <AlertTriangle className="text-amber-500" size={20} />
            <p className="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-tight">
              Límite alcanzado para el plan {plan}.
            </p>
          </div>
          <a
            href="/dashboard/settings"
