...
export default function BarbersPage() {
  ...
  const handleImageUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      // 1. Verificación inicial de peso (si es > 2MB ni lo intentamos procesar)
      if (file.size > 2000000) {
        alert(
          "La imagen es demasiado grande. Intenta con una de menos de 2MB."
        );
        return;
      }
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          // 2. Crear un canvas para redimensionar y comprimir
          const canvas = document.createElement("canvas");
          const MAX_WIDTH = 400; // Suficiente para una foto de perfil
          ...
          // 3. Convertir a Base64 con calidad reducida (0.7 = 70%)
