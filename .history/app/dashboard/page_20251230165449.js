...
export default function DashboardPage() {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [sentMessages, setSentMessages] = useState([]); // Tracking local de mensajes enviados
  ...
  // --- 3. LÓGICA DE FILTRADO Y SMART RECALL ---
  const { filteredAppointments, activeTodayCount, recallClients } =
    useMemo(() => {
      const allApps = barberiaData?.appointments || [];
      const hoyStr = getTodayStr();
      const hoyIndex = new Date().getDay() === 0 ? 6 : new Date().getDay() - 1;

      // Filtro para la agenda de hoy (Igual a tu original)
      const todayPending = allApps.filter((app) => {
        if (app.status !== "pending") return false;
        if (app.date) return app.date.substring(0, 10) === hoyStr;
        return app.day === hoyIndex;
      });

      // Smart Recall (20 días sin venir)
      const lastVisits = {};
      allApps.forEach((app) => {
  ...
  const handleRecallClick = (phone, name, clientId) => {
    ...
    const message = encodeURIComponent(
      `¡Hola ${name}! 💈 Hace tiempo no te vemos por la barbería. ¿Te gustaría agendar un lugar para esta semana?\n\nAgendate online aquí: ${bookingLink}`
    );
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-2xl transition-all ${
                      isSent
                        ? "opacity-30 bg-slate-100 grayscale"
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        handleRecallClick(client.phone, client.name, client.id)
                      }
                      className={`p-2.5 rounded-xl ${
                        isSent
                          ? "bg-slate-200 text-slate-400"
                          : "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:scale-105 transition-all"
                      }`}
                    >
                      {isSent ? (
                        <Check size={14} />
