...
import {
  ...
  Zap,
  Award,
  Check, // Nuevo icono para confirmar envío
} from "lucide-react";
// Firebase
import { auth, db } from "@/services/firebase";
import { onAuthStateChanged } from "firebase/auth";
...
export default function DashboardPage() {
  ...
  // Estado para marcar mensajes enviados en la sesión actual
  ...
  const handleAddAppointment = async (e) => {
    ...
    try {
      ...
      const newList = [...(barberiaData?.appointments || []), newApp];
      await updateDoc(barberiaRef, { appointments: newList });
      setBarberiaData((prev) => ({ ...prev, appointments: newList }));
      setIsModalOpen(false);
      ...
      if (phone) {
        ...
        const bookingLink = `http://barberias.vercel.app/reserva/${uid}`;
        const msg = encodeURIComponent(
  ...
  const handleCompleteAppointment = async (appointmentId) => {
    try {
      const barberiaRef = doc(db, "barberias", auth.currentUser.uid);
      const newList = barberiaData.appointments.map((app) =>
  ...
  // Función para manejar el clic en Smart Recall
  const handleRecallClick = (phone, name, clientId) => {
    const formattedPhone = formatUruguayPhone(phone);
    const bookingLink = `http://barberias.vercel.app/reserva/${auth.currentUser?.uid}`;
    const message = encodeURIComponent(
      `¡Hola ${name}! 💈 Notamos que hace tiempo no pasas por la barbería. ¿Te gustaría asegurar tu lugar para esta semana?\n\nAgendate online aquí: ${bookingLink}`
    );

    window.open(`https://wa.me/${formattedPhone}?text=${message}`, "_blank");

    // Agregamos a la lista de "enviados" para cambiar el color
    setSentMessages((prev) => [...prev, clientId]);
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950">
      <header className="flex-none flex items-center justify-between px-8 py-6 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
        ...
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-2xl shadow-lg shadow-blue-500/20 font-black text-xs uppercase tracking-widest ml-4"
        >
          <Plus size={18} /> Nueva Reserva
        </button>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard
            title="Suscripción"
            value={barberiaData?.plan?.type || "Pro"}
            change="Activo"
            icon={<ShieldCheck size={16} />}
            color="emerald"
          />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* SIDEBAR DE MARKETING CON CAMBIO DE COLOR */}
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm">
              {recallClients.length > 0 ? (
                recallClients.slice(0, 8).map((client) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-2xl transition-all duration-500 ${
                        isSent
                          ? "opacity-30 bg-slate-100 dark:bg-slate-800 grayscale"
                          : "bg-slate-50 dark:bg-slate-800/50 shadow-sm"
                      }`}
                    >
                      <div className="truncate pr-2">
                        <p
                          className={`text-[11px] font-black uppercase dark:text-white truncate ${
                            isSent ? "line-through" : ""
                          }`}
                        >
                          {client.name}
                        ...
                        <p className="text-[8px] font-bold text-slate-500 uppercase">
                          Vino: {client.date.toLocaleDateString("es-ES")}
                        </p>
                      </div>
                      <button
                        onClick={() =>
                        ...
                        className={`p-2.5 rounded-xl transition-all ${
                          isSent
                            ...
                            : "bg-emerald-500 text-white hover:scale-110 shadow-lg shadow-emerald-500/20"
                        }`}
                      >
                        {isSent ? (
    ...
    <div className="flex items-center gap-4 p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.8rem] hover:border-blue-500 transition-all shadow-sm">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] font-bold text-slate-400 uppercase mt-1.5">
          Corte con <span className="text-blue-500">{app.barber}</span>
        </p>
      </div>
      <div className="flex items-center gap-2">
        <button
