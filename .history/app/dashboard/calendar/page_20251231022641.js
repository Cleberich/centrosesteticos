...
export default function CalendarPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0a0f1a]">
      ...
      {/* DRAWER */}
      {isDrawerOpen && (
        <div className="fixed top-0 inset-0 z-[100] flex justify-end items-end md:items-stretch">
          <div
            className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"
            onClick={() => setIsDrawerOpen(false)}
          />
          <div className="relative w-full md:max-w-[450px] bg-white dark:bg-slate-900 h-[90vh] md:h-full rounded-t-[2.5rem] md:rounded-none flex flex-col p-6 md:p-8 overflow-y-auto">
            <div className="w-12 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full mx-auto mb-6 md:hidden" />
            ...
            {!showPaymentSelector ? (
              <form onSubmit={handleSave} className="space-y-5 pb-10 mt-12">
                {currentApp.id && currentApp.status !== "done" && (
                  <button
                    type="button"
                    onClick={() => setShowPaymentSelector(true)}
