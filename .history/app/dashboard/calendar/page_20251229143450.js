...
// ... (Constantes HOUR_HEIGHT, START_HOUR, DAYS, BARBER_COLORS, PAYMENT_METHODS se mantienen igual)
...
export default function CalendarPage() {
  ...
  // --- FUNCIÓN PARA LIBERAR / ELIMINAR TURNO ---
  const handleDelete = async () => {
    if (
      !window.confirm(
        "¿Estás seguro de que deseas cancelar y liberar este turno?"
      )
    )
      return;
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0a0f1a]">
      {/* HEADER Y CALENDARIO (Se mantiene igual que tu código) */}
      ...
      {/* DRAWER ACTUALIZADO CON BOTÓN ELIMINAR */}
      {isDrawerOpen && (
          ...
          <div className="relative w-full max-w-[450px] bg-white dark:bg-slate-900 h-full flex flex-col p-8 overflow-y-auto animate-in slide-in-from-right shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex items-center gap-2">
                {/* BOTÓN DE ELIMINAR (Solo si existe y no está cobrado) */}
                {currentApp.id &&
