...
export default function CustomersPage() {
    ...
    return Object.values(customerMap)
      .map((c) => {
        ...
        if (dias > 60 && c.visits > 1) {
          ...
          status = "Frequent";
          statusColor = "bg-[#EEF7F2] text-[#3B7A57] border-[#D8ECE0]";
        }
        ...
        return {
          ...
          lastVisitDisplay:
            c.lastVisitDate.getTime() === 0
              ? "---"
              : c.lastVisitDate.toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "2-digit",
                  day: "2-digit",
                }),
  ...
  if (loading && !esteticaData)
    return (
      <div className="flex-1 flex items-center justify-center bg-[#FAF8F5] dark:bg-slate-950 h-screen">
  ...
  return (
    <div className="flex flex-col h-full bg-[#FAF8F5] dark:bg-slate-950 font-sans text-slate-700 dark:text-slate-200">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between px-8 py-6 gap-4 bg-transparent">
        <div>
          <h1 className="text-2xl font-semibold text-slate-800 dark:text-white tracking-tight">
            Patients
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Total registered patients: {filteredCustomers.length}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          <div className="relative flex-1 md:w-72">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-full text-xs outline-none focus:border-emerald-400 transition-all shadow-sm placeholder:text-slate-400"
            />
          </div>
          <button className="p-2.5 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-full text-slate-500 dark:text-slate-400 hover:border-emerald-400 transition-all shadow-sm shrink-0">
            <Filter size={16} />
          </button>
          <button
            onClick={() => {
      ...
      <div className="flex-1 overflow-auto px-8 pb-8">
        <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] border border-slate-200/60 dark:border-slate-800/80 shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800 text-slate-400 font-medium">
                <th className="pl-6 py-4 font-normal">Patient Name</th>
                <th className="px-4 py-4 font-normal">Contact</th>
                <th className="px-4 py-4 font-normal">Last Visit</th>
                <th className="px-4 py-4 font-normal text-center">Sessions</th>
                ...
                >
                  ...
                  <td className="pl-6 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="size-9 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 flex items-center justify-center text-xs font-medium text-slate-600 dark:text-slate-300 shrink-0">
                        {customer.name.substring(0, 2).toUpperCase()}
                      </div>
                      <span className="font-semibold text-slate-800 dark:text-white group-hover:text-emerald-600 transition-colors">
                        {customer.name}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3.5">
                    <span
                      className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-medium border ${customer.statusColor}`}
                    >
                      {customer.status}
                    </span>
                  </td>
                  ...
                  <td className="pr-6 py-3.5 text-right">
                    <ChevronRight
                      size={16}
                      className="inline text-slate-300 dark:text-slate-600 group-hover:text-slate-500 transition-transform group-hover:translate-x-0.5"
                    />
                  </td>
                </tr>
              ))}
      ...
      {isEditOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div
            ...
            onClick={() => setIsEditOpen(false)}
          />
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 h-full shadow-2xl p-6 flex flex-col justify-between border-l border-slate-100 dark:border-slate-800">
            <div>
              {/* Header del Drawer */}
              ...
              >
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 rounded-xl text-xs outline-none focus:border-emerald-500 text-slate-800 dark:text-white"
                ...
                {selectedCustomer && (
                  <div className="grid grid-cols-2 gap-3 pt-4 mt-6 border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
              <button
                type="submit"
                form="patient-form"
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-medium transition-all shadow-sm"
              >
