...
export default function DashboardPage() {
  ...
  const { filteredAppointments, activeTodayCount } = useMemo(() => {
    ...
    // Generamos la fecha de hoy: "2025-12-30"
    const hoy = new Date().toLocaleDateString("sv-SE");

    // 1. Filtrado con limpieza de datos
    const todayPending = allApps.filter((app) => {
      // Limpiamos la fecha de la DB por si tiene espacios o formatos raros
      const appDate = app.date ? String(app.date).trim().split("T")[0] : "";
      const esPendiente = app.status === "pending";
      const esDeHoy = appDate === hoy;

      return esPendiente && esDeHoy;
    });
    ...
    // Debug en consola para que veas qué está fallando
    console.log("Fecha hoy sistema:", hoy);
    console.log("Citas totales encontradas:", allApps.length);
    console.log("Citas que pasaron el filtro de hoy:", todayPending.length);
