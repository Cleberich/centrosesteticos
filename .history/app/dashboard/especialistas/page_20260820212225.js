...
export default function SpecialistsByutiePage() {
  ...
  const handleOpenCreate = () => {
    ...
    if (activeSpecialists.length >= (PLAN_LIMITS[plan] || 1)) {
      alert(`Tu plan ${plan} tiene el cupo lleno.`);
      return;
    }
    setFormData({
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#FAF8F5] overflow-y-auto">
        ...
        <div className="px-10 py-2 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Plus size={15} /> Añadir Especialista
            </button>
          </div>
        </div>
        ...
        <div className="p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredSpecialists.map((s) => (
            <div
              key={s.id}
              className={`bg-white border rounded-3xl p-6 flex flex-col items-center text-center shadow-xs transition-shadow relative group ${s.active === false ? "border-amber-200 bg-amber-50/40 opacity-75" : "border-slate-200/70 hover:shadow-md"}`}
            >
              {/* Botón Editar rápido */}
              <button
                onClick={() => s.active !== false && handleOpenEdit(s)}
