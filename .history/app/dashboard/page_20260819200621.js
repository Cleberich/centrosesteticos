...
export default function EsteticaDashboard() {
  ...
  // --- ESTADOS ---
  const [esteticaData, setEsteticaData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  const formatUruguayPhone = (phone: string) => {
    if (!phone) return "";
    let clean = phone.replace(/\D/g, "");
    if (clean.startsWith("0")) {
      clean = "598" + clean.substring(1);
  ...
  const handleAddAppointment = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isInactive) {
      alert("Acceso restringido: Debes regularizar tu pago para agendar citas.");
      return;
    }
    ...
    const specialist = formData.get("specialistName") as string;
    ...
    const newApp = {
      ...
      phone: phone,
      specialist: specialist,
      serviceCategory: serviceCategory || "Manicuría",
      start: time,
      date: getTodayStr(),
      status: "pending",
    };
    ...
    try {
      ...
      const newList = [...currentApps, newApp];
      await updateDoc(esteticaRef, { appointments: newList });
      setEsteticaData((prev: any) => ({ ...prev, appointments: newList }));
      setIsModalOpen(false);

      if (phone) {
        const formattedPhone = formatUruguayPhone(phone);
        const msg = encodeURIComponent(
          `Hola ${name}, confirmamos tu cita de ${serviceCategory} para hoy a las ${time}hs. ✨ ¡Te esperamos!`
        );
        window.open(`https://wa.me/${formattedPhone}?text=${msg}`, "_blank");
      }
    } catch (error) {
  ...
  const confirmPaymentAndComplete = async (paymentMethod: string) => {
    if (!appointmentToComplete || !auth.currentUser) return;
    try {
      const esteticaRef = doc(db, "centros_estetica", auth.currentUser.uid);
      const currentApps = esteticaData?.appointments || [];
      const newList = currentApps.map((app: any) =>
        String(app.id) === String(appointmentToComplete.id)
          ? {
              ...app,
              status: "done",
  ...
  const handleDeleteAppointment = async (appointmentId: string) => {
    ...
    try {
      const esteticaRef = doc(db, "centros_estetica", auth.currentUser.uid);
      const newList = esteticaData.appointments.filter(
        (app: any) => String(app.id) !== String(appointmentId)
      );
      await updateDoc(esteticaRef, { appointments: newList });
      setEsteticaData((prev: any) => ({ ...prev, appointments: newList }));
    } catch (error) {
  ...
  const handleSendReminder = (app: any) => {
    ...
    const formattedPhone = formatUruguayPhone(app.phone);
    const msg = encodeURIComponent(
      `Hola ${app.customer}, recordatorio de tu turno de ${app.serviceCategory || "belleza"} hoy a las ${app.start}hs 💅🌸`
    );
    window.open(`https://wa.me/${formattedPhone}?text=${msg}`, "_blank");
  };
  ...
  const { filteredAppointments, recallClients } = useMemo(() => {
    ...
    const today = allApps.filter(
      (app: any) => app.status === "pending" && app.date === hoyStr
    );
    ...
    const limitDate = new Date();
    limitDate.setDate(limitDate.getDate() - 21); // Ciclo retoque uñas/pestañas (21 días)
    const recall = Object.values(lastVisits).filter(
      (client) => client.date < limitDate
    );
    ...
    const filtered = today
      .filter((app: any) => {
        ...
        const matchesCat =
          selectedCategory === "Todos" || app.serviceCategory === selectedCategory;
  ...
  const categories = ["Todos", "Peluquería", "Manicuría", "Pestañas", "Estética"];
      ...
      <aside className="w-64 bg-white/80 backdrop-blur-md border-r border-slate-100 flex flex-col justify-between p-6 shrink-0">
        <div className="space-y-8">
          ...
          <nav className="space-y-1.5">
            ...
            <NavItem icon={<CalendarIcon size={18} />} label="Agenda de Turnos" />
            <NavItem icon={<Users size={18} />} label="Clientes" />
            <NavItem icon={<Scissors size={18} />} label="Servicios" />
            <NavItem icon={<PaymentsIcon size={18} />} label="Caja y Pagos" />
          </nav>
        </div>
          ...
          <div className="bg-[#FFF0ED] p-4 rounded-3xl border border-[#FDE2E4]/60 relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-[10px] text-slate-500 leading-relaxed mb-3">
                Envía recordatorios automáticos de retoque de pestañas y uñas.
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full font-bold text-xs transition-all flex items-center gap-2 shadow-sm ${
                isInactive
                  ...
                  : "bg-[#67C3A5] hover:bg-[#52B293] active:scale-95 text-white shadow-[#67C3A5]/20"
              }`}
            >
              {isInactive ? <Lock size={14} /> : <Plus size={16} />}
              Nuevo Turno
            </button>
        ...
        <div className="p-8 space-y-8 max-w-7xl">
          ...
          {isInactive && (
            <div className="bg-[#FFF0ED] border border-[#FFC2B4] p-4 rounded-2xl flex items-center gap-4 text-rose-700">
              ...
              <div className="text-xs">
                <p className="font-bold uppercase tracking-wider">Cuenta Inactiva</p>
                <p className="text-slate-600">
                  Regulariza la suscripción para agendar nuevos turnos.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  <Sparkles size={16} className="text-[#67C3A5]" /> Turnos
                ...
                {filteredAppointments.length > 0 ? (
                  ...
                  <div className="p-12 text-center bg-white rounded-3xl border border-dashed border-slate-200">
                    <p className="text-xs font-semibold text-slate-400">
                      No hay citas registradas bajo esta categoría hoy
                    </p>
                  </div>
                )}
              </div>
            ...
            {/* COLUMNA DERECHA: RECALL / FIDELIZACIÓN */}
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <Heart size={16} className="text-[#F89B88]" /> Recordatorios de
                Retoque
              </h3>

              <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-3">
                {recallClients.length > 0 ? (
                  recallClients.slice(0, 5).map((client: any) => {
                    const isSent = sentMessages.includes(client.id);
                    return (
                      <div
                        key={client.id}
                      ...
                      >
                        <div className="truncate pr-2">
                          ...
                          <p className="text-[10px] text-[#F89B88] font-medium">
                            Último: {client.service} ({client.date.toLocaleDateString()})
                          </p>
                        </div>
                        <button
                          onClick={() => {
                            const formatted = formatUruguayPhone(client.phone);
                            const msg = encodeURIComponent(
                              `¡Hola ${client.name}! ✨ Pasaron 3 semanas desde tu último servicio de ${client.service}. ¿Agendamos tu retoque? 💅🌸`
                            );
                            window.open(
                              `https://wa.me/${formatted}?text=${msg}`,
                              "_blank"
                        ...
                        >
                          {isSent ? <Check size={14} /> : <MessageSquare size={14} />}
                        </button>
                      </div>
                    );
                  })
          ...
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 border border-slate-100">
            <div className="text-center mb-6">
              ...
              <p className="text-slate-400 text-xs mt-1">
                Cliente: {appointmentToComplete.customer}
              </p>
            </div>
            ...
            <div className="grid gap-2.5">
              {[
                ...
                { id: "transfer", icon: <SmartphoneNfc size={16} />, label: "Transferencia" },
                { id: "mp", icon: <CreditCard size={16} />, label: "Mercado Pago" },
                { id: "pos", icon: <CreditCard size={16} />, label: "POS / Tarjeta" },
              ].map((m) => (
                <button
                  key={m.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-[#FAF8F5]">
              <h3 className="font-bold text-slate-800 text-sm">Agendar Cita en Studio</h3>
              <button
...
// --- SUBCOMPONENTES AUXILIARES ESTILO BYUTIE ---
...
function ByutieStatCard({ title, value, badge, icon, bgColor }: any) {
  return (
...
function ByutieAppointmentRow({ app, onDelete, onComplete, onReminder }: any) {
  // Colores por categoría estilo pastel
  ...
  const badgeStyle = categoryColors[app.serviceCategory] || "bg-slate-100 text-slate-600";
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-3xl hover:border-slate-200 transition-all shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full ${badgeStyle}`}>
              {app.serviceCategory || "Tratamiento"}
            </span>
          </div>
          <p className="text-[10px] text-slate-400 font-medium mt-0.5">
            Especialista: <span className="text-slate-600 font-semibold">{app.specialist}</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button
          onClick={onComplete}
          title="Marcar completado / Cobrar"
