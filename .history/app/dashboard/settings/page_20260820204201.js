    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        try {
          ...
          const malformedPreapproval = searchParams
            .get("payment_status")
            ?.match(/(?:\?|&)preapproval_id=([^&]+)/)?.[1];
          const preapprovalId =
            searchParams.get("preapproval_id") || malformedPreapproval;
          const paymentStatus =
        ...
        <form onSubmit={handleSave} className="space-y-8">
          ...
          <section className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-rose-100/60 shadow-sm space-y-2">
              ...
              <p className="text-xs text-slate-400">
                Puede ser diferente al email con el que te registraste en la
                aplicación.
              </p>
              <input
