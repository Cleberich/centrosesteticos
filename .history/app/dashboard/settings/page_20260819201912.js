...
function SettingsContent() {
  ...
  const planes = [
    {
      ...
      features: ["1 Especialista", "50 citas/mes", "Peluquería o Manicura"],
    },
    {
      id: "Radiance",
      name: "Radiance (Pro)",
    ...
    },
    {
      ...
      features: [
        "Especialistas Ilimitados",
        "Citas ilimitadas",
        "Peluquería, Manicura y Pestañas",
        "Módulo de Finanzas con PIN",
  ...
  return (
    <div className="min-h-screen bg-[#FBF8F6] font-sans text-slate-700 flex">
      {/* SIDEBAR INSPIRADO EN BYUTIE */}
      <aside className="w-64 bg-white border-r border-rose-100/60 p-6 flex flex-col justify-between hidden md:flex">
        <div className="space-y-8">
          <div className="flex items-center gap-3 px-2">
            <div className="size-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold">
              ✦
            </div>
          ...
          <nav className="space-y-1">
            {[
              ...
              { label: "Especialistas", icon: <UserCheck size={18} /> },
              { label: "Citas", icon: <Calendar size={18} /> },
              { label: "Servicios", icon: <Scissors size={18} /> },
              { label: "Pagos", icon: <CreditCard size={18} /> },
              { label: "Mensajes", icon: <MessageSquare size={18} /> },
            ].map((item, idx) => (
              <button
                key={idx}
                className="w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold rounded-2xl text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition-all"
              >
                {item.icon}
                {item.label}
              </button>
        ...
        <div className="bg-rose-100/50 p-5 rounded-3xl space-y-3 border border-rose-200/40">
          <p className="text-xs font-bold text-slate-800">
            Gestión de Citas Pro
          </p>
          <p className="text-[11px] text-slate-500 leading-relaxed">
            Optimiza los turnos de manicura y peluquería en tiempo real.
          </p>
          <button className="bg-emerald-200 text-emerald-800 px-4 py-2 rounded-xl text-[10px] font-bold w-full hover:bg-emerald-300 transition-colors">
            Ver Calendario
          </button>
        </div>
      </aside>
          ...
          <section className="bg-white p-8 rounded-[2rem] border border-rose-100/60 shadow-sm space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 bg-slate-50 border border-slate-100 p-3.5 rounded-2xl text-xs font-mono text-slate-600 truncate">
                {`https://byutie-estetica.vercel.app/reserva/${auth.currentUser?.uid}`}
              </div>
              <button
                type="button"
                onClick={() => {
