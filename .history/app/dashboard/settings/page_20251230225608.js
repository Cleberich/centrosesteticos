...
//                 <Lock size={32} />
//               </div>
//               <h3 className="text-xl font-black uppercase dark:text-white tracking-tight italic">
//                 Desactivar PIN
//               </h3>
...
import {
  ...
  Navigation, // Icono para el link del mapa
} from "lucide-react";
import { auth, db } from "@/services/firebase";
import { onAuthStateChanged, signOut } from "firebase/auth";
import { doc, getDoc, updateDoc } from "firebase/firestore";
...
function SettingsContent() {
  ...
  // --- FUNCIÓN PARA BUSCAR DIRECCIÓN EN GOOGLE MAPS ---
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <section className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="md:col-span-2 space-y-3">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest flex items-center gap-2">
                <MapPin size={12} /> Dirección de la Barbería
              </label>
              <div className="flex gap-2">
                <input
                ...
                <button
                  ...
                  className="px-6 bg-slate-900 dark:bg-slate-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 transition-colors"
                >
                  Buscar en Mapa
                </button>
              </div>
            ...
            <div className="md:col-span-2 space-y-3">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest flex items-center gap-2">
                <Navigation size={12} /> Link Directo de Google Maps (URL)
              </label>
              <input
                placeholder="Pega aquí el link de 'Compartir' de Google Maps"
                value={barberiaData?.mapsLink || ""}
                onChange={(e) =>
                  setBarberiaData({ ...barberiaData, mapsLink: e.target.value })
                }
                className="w-full bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl font-bold dark:text-white outline-none border dark:border-slate-700 text-blue-500"
              />
              <p className="text-[9px] text-slate-500 font-bold uppercase ml-2 italic">
                * Recomendado para que tus clientes lleguen por GPS sin errores.
              </p>
            </div>
          </section>
          ...
          <section className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="flex items-center gap-2 font-black uppercase text-xs text-yellow-500 tracking-widest italic">
                  <ShieldCheck size={18} /> Seguridad de Finanzas
                </h3>
                <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">
                  Bloquea la sección de contabilidad
                </p>
              </div>
              <div
                onClick={handleTogglePinProtection}
            ...
            {barberiaData?.useAccountingPin && (
              <input
                ...
                className="w-full max-w-xs bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-blue-500/30 rounded-2xl py-4 px-6 text-center text-xl font-black tracking-[.5em] text-blue-600 outline-none focus:border-blue-500"
              />
            )}
          </section>
        ...
        {/* MODAL VERIFICACIÓN PIN (Omitido por brevedad pero integrado en la lógica) */}
        {isVerifyModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 border dark:border-slate-800 text-center">
              ...
              <form onSubmit={confirmDeactivationAndSave} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-4 bg-red-600 text-white rounded-xl font-black text-[10px] uppercase"
                >
                  Desbloquear y Guardar
                </button>
              </form>
            </div>
          </div>
