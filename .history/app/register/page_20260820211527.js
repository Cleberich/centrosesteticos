...
export default function RegisterPage() {
  ...
  const addSpecialist = () => {
    if (specialists.length >= 1) {
      alert("El plan Inicial permite 1 especialista. Actualiza tu plan para agregar más.");
      return;
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // El plan Inicial es gratuito y solo habilita una especialista.
      const configuredSpecialists = specialists.slice(0, 1).map((specialist) => ({
        ...specialist,
