...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const configuredSpecialists = specialists.map((specialist, index) => ({
        ...specialist,
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            {step === 4 && <div className="space-y-5">
              ...
              <div className="space-y-4">
                {services.map((service) => <div key={service.id} className="rounded-2xl border border-slate-200 p-4 space-y-3"><div className="flex gap-2"><input value={service.name} onChange={(event) => updateService(service.id, "name", event.target.value)} placeholder="Ej. Manicura semipermanente" className="flex-1 rounded-xl border border-slate-200 p-3 text-xs outline-none focus:border-emerald-400" /><button type="button" onClick={() => setServices((current) => current.filter((item) => item.id !== service.id))} className="p-2 text-rose-400" aria-label="Eliminar servicio"><Trash2 size={16} /></button></div><select value={service.category} onChange={(event) => updateService(service.id, "category", event.target.value)} className="w-full rounded-xl border border-slate-200 bg-white p-3 text-xs outline-none">{(categories.length ? categories : categoryOptions).map((category) => <option key={category}>{category}</option>)}</select><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">¿Quién realiza este servicio?</p>{specialists.map((specialist) => { const assigned = service.specialistIds?.includes(specialist.id); return <div key={specialist.id} className="grid grid-cols-[auto_1fr_90px_90px] items-center gap-2"><input type="checkbox" checked={assigned} onChange={() => toggleServiceSpecialist(service.id, specialist.id)} className="h-4 w-4 accent-emerald-500" /><span className="truncate text-xs font-semibold text-slate-600">{specialist.name || "Especialista"}</span><input type="number" min="1" disabled={!assigned} value={service.specialistPrices?.[specialist.id] || ""} onChange={(event) => updateServiceSpecialistValue(service.id, specialist.id, "specialistPrices", event.target.value)} placeholder="$ Precio" className="rounded-xl border border-slate-200 p-2.5 text-xs outline-none focus:border-emerald-400 disabled:bg-slate-100" /><div className="relative"><Clock3 size={13} className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" /><input type="number" min="1" disabled={!assigned} value={service.specialistTimes?.[specialist.id] || ""} onChange={(event) => updateServiceSpecialistValue(service.id, specialist.id, "specialistTimes", event.target.value)} placeholder="Min" className="w-full rounded-xl border border-slate-200 p-2.5 pl-7 text-xs outline-none focus:border-emerald-400 disabled:bg-slate-100" /></div></div>; })}</div>)}
              </div>
