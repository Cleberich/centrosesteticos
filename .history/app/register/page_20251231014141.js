...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // ---------------------------------------------------------
      // 2. ENVIAR NOTIFICACIÓN POR EMAIL (Nuevo código)
      // ---------------------------------------------------------
      try {
        await emailjs.send(
          ...
          "YOUR_TEMPLATE_ID", // Reemplaza con tu Template ID de EmailJS
          {
            ...
            email: formData.email, // Email del usuario registrado
            phone: formData.phone,
          },
        ...
        // No bloqueamos el registro si falla el email, solo lo logueamos
        console.error("No se pudo enviar la notificación:", emailError);
      }
      // ---------------------------------------------------------
