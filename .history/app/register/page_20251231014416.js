...
//             <h1 className="text-2xl flex gap-2 font-black uppercase dark:text-white tracking-tighter">
//               Agenda{" "}
//               <div className="size-10 -mt-2  bg-blue-600 rounded-xl flex items-center justify-center text-white">
//                 <Scissors size={24} />
//               </div>
//               Barber
//             </h1>
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // 3. ENVIAR EMAIL DE NOTIFICACIÓN (Nuevo código)
      // Esto intenta enviar el correo, si falla, solo muestra error en consola pero deja pasar al usuario
      try {
        await emailjs.send(
          "service_bm0xbov", // Pega aquí tu Service ID de EmailJS
          "template_ixhkgam", // Pega aquí tu Template ID de EmailJS
          {
          ...
          "QQV_D_IXpW03jTg8X" // Pega aquí tu Public Key de EmailJS
        );
        console.log("Email de notificación enviado correctamente.");
      } catch (emailError) {
        console.error(
          "Error enviando el email (el registro sí funcionó):",
          emailError
        );
      }
