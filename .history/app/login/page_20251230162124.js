...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Verificamos si el email coincide con el de la variable .env
      const adminEmail = process.env.NEXT_PUBLIC_ADMIN_EMAIL;

      if (user.email === adminEmail) {
        // Si es el Super Admin, va a su panel especial
        router.push("/admin/control");
      } else {
        // Si es un barbero normal, va a su dashboard
        router.push("/dashboard");
      }
    } catch (err) {
      // IMPORTANTE: No imprimas 'err' completo en la consola para evitar errores de stack
