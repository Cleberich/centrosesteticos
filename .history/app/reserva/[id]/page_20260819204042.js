...
export default function PublicBookingPage() {
  ...
  const handleConfirm = async (e) => {
    ...
    if (booking.phone.length < 11)
      return alert("El teléfono debe ser 09X XXX XXX");
    if (!booking.email) return alert("Ingresa tu email para recibir la confirmación");
    ...
    try {
      ...
      try {
        ...
        alert("La reserva se guardó, pero no pudimos enviar el email de confirmación.");
      }
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-rose-100/60 py-4 px-6 sticky top-0 z-20 shadow-sm">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-bold text-emerald-600 tracking-wider uppercase block leading-none">
                Salón & Beauty
              </span>
